...
trait Votable
{
    ...
    {
        return $builder->withSum('votes as COALESCE(total_votes)', 'votes');
    }

    public function scopeWithTotalUpVotes(Builder $builder)
    {
        return $builder->withSum(['votes as COALESCE(total_up_votes, 0)' => fn ($q) => $q->where('votes', '>', 0)], 'votes');
    }

    public function scopeWithTotalDownVotes(Builder $builder)
    {
        return $builder->withSum(['votes as COALESCE(total_down_votes, 0)' => fn ($q) => $q->where('votes', '<', 0)], 'votes');
    }
